...
func (n *Parser) checkFiles() {
	storedFiles, err := db.Search(db.FILE_CHECKSUM_INDEX, db.FILE_CHECKSUM_TYPE, `{}`)
	if err != nil {
		log.Fatal(err)
	}

	var checksums []Checksum
	for _, r := range storedFiles {
		var ch Checksum
		b, ok := r.(json.RawMessage)
		if !ok {
			break
		}
	...
	for k, v := range n.files {
		...
		b, err := json.Marshal(NewChecksum(checksum))
		if err != nil {
			log.Fatal("Error new checksum", err)
		}

		db.Index(checksum, db.FILE_CHECKSUM_INDEX, db.FILE_CHECKSUM_TYPE, string(b))
		n.md5.Reset()
	}
	...
	for {
		...
		go func(counter int, record []string) {
			if counter == 0 {
				...
				db.Index("", db.DEFAULT_INDEX, db.DEFAULT_TYPE, string(json))
			}
